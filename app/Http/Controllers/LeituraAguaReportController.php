        ...
        $results = DB::select(
            ...
            LEFT JOIN (
                ...
                JOIN condomino c ON sub_lav.condomino = c.id
                        ...
                        AND EXTRACT(YEAR_MONTH FROM sub.dataleitura) = EXTRACT(YEAR_MONTH FROM DATE_SUB(la.dataleitura, INTERVAL 1 MONTH))
            WHERE TRUE 
                   AND DATE_SUB(la.dataleitura, INTERVAL 1 MONTH) BETWEEN :dataInicial AND :dataFinal $condomino
